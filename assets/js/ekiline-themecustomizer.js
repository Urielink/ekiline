...
function actualizarTema(){
	...
	// 2) Por cada color.
	misColores.forEach(function(color){
		// 2.1) Actualiza textarea con CSS.
		wp.customize( color.id, 'ekiline_textarea_css', function( field1, field2 ) {
...
/**
 * 3) Modificar colores.
 * @param {*} tonos recibe el arreglo de colores bootstrap.
 * @returns Devuelve arreglo con valores de cada color modificado.
 */
function proColores(tonos){

	var objTonoExt = {};

	tonos.forEach(function(color){

		var absoluto = wp.customize.value( color.id )();
		...
		objTonoExt[ color.txt ] = item;

	});

	// Prueba de salida
	// console.log( objTonoExt );
	return objTonoExt;
}

/**
 * 3.1) Formulas para cambio de colores.
 ...
 * @param {*} rgba recibe escalas numericas
 * @returns devuleve HEX con opacidad aproximada.
 */
...
/**
 * Texto CSS con valores de bootstrap, solo modifica color.
 * @param {*} extraccion Recibe arreglo con valores de color nuevos.
 * @returns Devuelve CSS para sobreescribir en input (ekiline_textarea_css).
 */
