...
const themeColors=[{name:'--bs-primary',id:'b4_primary'},{name:'--bs-secondary',id:'b4_secondary'},{name:'--bs-success',id:'b4_success'},{name:'--bs-info',id:'b4_info'},{name:'--bs-warning',id:'b4_warning'},{name:'--bs-danger',id:'b4_danger'},{name:'--bs-light',id:'b4_light'},{name:'--bs-dark',id:'b4_dark'},{name:'--bs-body-bg',id:'background_color'},{name:'--bs-body-color',id:'text_color'}];const coloresVariables=['alert-link','alert','btn-active','link-hover','btn-hover','form-control-focus','form-range-active','table-active-bg','table-hover-bg','table-striped-bg','table-bg'];updateFieldWithCss(themeColors,coloresVariables);function updateFieldWithCss(arrayColors,colorVariables){arrayColors.forEach(function(itemColor){wp.customize(itemColor.id,'ekiline_textarea_css',function(field1,field2){field1.bind(function(item){item=coloresHexadecimales(arrayColors);item=crearColores(item,colorVariables);item=construirCssRoot(item);item=item+construirCssGeneral();field2.set(item);});});});}
function coloresHexadecimales(arreglo){let nuevoArreglo=[];arreglo.forEach(function(item){let nombre=item.name;let color=wp.customize.value(item.id)();nuevoArreglo.push({name:nombre,hexCol:color});});return nuevoArreglo;}
function crearColores(arreglo,variables){let colorAll=[];for(var color of arreglo){colorAll=colorAll.concat(tonosVariables(color,variables));}
let nuevoArray=arreglo.concat(colorAll);nuevoArray=agregarTonosRGB(nuevoArray);return nuevoArray;}
function tonosVariables(color,tonos,media=-25){let colorVars=[],newColor='',sufijo='',newName='';for(let index=0;index<tonos.length;index++){if(index>0){media=media+5;}
if(index>4){if(index===5){media=25;}
if(index>6){media=media-5+3;}}
newColor=HexAHslvar(color.hexCol,0,0,media);sufijo='-'+tonos[index];newName=color.name+sufijo;colorVars.push({name:newName,hexCol:newColor});}
return colorVars;}
function agregarTonosRGB(arreglo){const newValRgb=[];for(var color of arreglo){newValRgb.push({name:color.name,hexCol:color.hexCol,alias:color.name+'-rgb',rgbCol:hexToRgb(color.hexCol)});}
return newValRgb;}
