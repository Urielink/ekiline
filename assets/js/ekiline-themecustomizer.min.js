function actualizarTema(){var misColores=[{txt:'primary',id:'b4_primary'},{txt:'secondary',id:'b4_secondary'},{txt:'success',id:'b4_success'},{txt:'danger',id:'b4_danger'},{txt:'warning',id:'b4_warning'},{txt:'info',id:'b4_info'},{txt:'light',id:'b4_light'},{txt:'dark',id:'b4_dark'},{txt:'background',id:'background_color'},{txt:'text',id:'text_color'}];misColores.forEach(function(color){wp.customize(color.id,'ekiline_textarea_css',function(field1,field2){field1.bind(function(item){item=cssOrders(proColores(misColores));field2.set(item);});});});}
actualizarTema();function proColores(tonos){var objTonoExt={};tonos.forEach(function(color){var absoluto=wp.customize.value(color.id)();var aHColor=HexAHslvar(absoluto,0,0,-10);var btnHBdc=HexAHslvar(absoluto,0,0,-15);var btnNotBdc=HexAHslvar(absoluto,0,0,-20);var btnFocBx=rgbaToHexOp(hexToRgbA(HexAHslvar(absoluto,0,0,35)));var btnFocHBx=rgbaToHexOp(hexToRgbA(HexAHslvar(absoluto,0,0,10)));var altTx=rgbaToHexOp(hexToRgbA(HexAHslvar(absoluto,0,0,-20)));var altLk=rgbaToHexOp(hexToRgbA(HexAHslvar(absoluto,0,0,-35)));var nvLigNvTxc=rgbaToHexOp(hexToRgbA(absoluto,'.5'));var nvLigNvHTxc=rgbaToHexOp(hexToRgbA(absoluto,'.7'));var nvLigNvDisTxc=rgbaToHexOp(hexToRgbA(absoluto,'.3'));var thLigBgc=rgbaToHexOp(hexToRgbA(HexAHslvar(absoluto,0,0,20),'.20'));var tabBgc05=rgbaToHexOp(hexToRgbA(absoluto,'.05'));var tabBgc075=rgbaToHexOp(hexToRgbA(absoluto,'.075'));var crdBdc=rgbaToHexOp(hexToRgbA(absoluto,'.1'));var item={'puro':absoluto,'hover':aHColor,'borde':btnHBdc,'clic':btnNotBdc,'focus':btnFocBx,'focusHv':btnFocHBx,'alertTx':altTx,'alertLk':altLk,'navTx':nvLigNvTxc,'navHov':nvLigNvHTxc,'navDis':nvLigNvDisTxc,'breadBg':thLigBgc,'tabBg':tabBgc05,'tabBgHv':tabBgc075,'tabBd':crdBdc};objTonoExt[color.txt]=item;});return objTonoExt;}
function hexToRgbA(hex,op){var c;if(op==''||op==null){op=1;}
...
function rgbaToHexOp(rgba){var a,rgb=rgba.replace(/\s/g,'').match(/^rgba?\((\d+),(\d+),(\d+),?([^,\s)]+)?/i),alpha=(rgb&&rgb[4]||"").trim(),hex=rgb?(rgb[1]|1<<8).toString(16).slice(1)+(rgb[2]|1<<8).toString(16).slice(1)+(rgb[3]|1<<8).toString(16).slice(1):rgba;if(alpha!==""){a=alpha;}else{a=01;}
a=Math.round(a*100)/100;alpha=Math.round(a*255);var hexAlpha=(alpha+0x10000).toString(16).substr(-2).replace('ff','');hex=hex+hexAlpha;return'#'+hex;}
