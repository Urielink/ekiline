...
= 2.3.7 - May 9, 2022 =
* Update CSS styles modal.
* Fix: Adding classnames in thumbnail (template-parts directory.)
* Disable backdrop filter CSS.
...
* Update: Article pagination markup (theme-elements.php).
...
* CSS Fix: inherit text color in links when occupying in a block.
* CSS Fix: hide title (h1) with CSS styles (.screen-reader) when custom header is enabled.
* Admin fix: ekiline_theme_page update content and styles.
...
* CSS Fix: Add bootstrap font heading sizes (style.css).
...
* Clean the ekiline css styles from (assets/css/style-atf.css). if you need you can use it with your own css rules.
